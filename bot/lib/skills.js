    ...
    try {
      ...
      const sample = sampleFootprintGround({
        ...
        isSolidGroundBlock: (block, sampleCtx = {}) => {
          ...
          const sx = Number.isFinite(sampleCtx.x) ? sampleCtx.x : sampleCtx.wx;
          const sz = Number.isFinite(sampleCtx.z) ? sampleCtx.z : sampleCtx.wz;
          if (!isFoundationGroundBlock(bot, block, sx, sampleCtx.y, sz)) return false;
          const bn = block.name || '';
