...
export function isSpeculativeBuildDiscussion(body) {
  ...
  if (!/\b(build|building|construct|design|observatory|wizard tower|market square|sky bridge|beacon plaza|schematic)\b/.test(text)) {
    return false;
  }
  return /\b(should we|someday|some day|later|another day|wish we could|tell me a story|talked about|do you remember|remember when)\b/.test(text)
    ...
    || /\bwhere (is|are) (the |our )?(sky bridge|beacon plaza|wizard tower|crystal observatory|market square|grand hotel|treehouse)\b/.test(text)
